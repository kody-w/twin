...
// The signed/hashed core = every frame field EXCEPT sha and sig.
export function frameCore(frame) {
  const { sha, sig, ...core } = frame;
  return core;
}
...
export function buildSignedFrame(draft) {
  ...
  const sig = signCanonical(canonical, loadPrivateKey());

  const frame = { sha, prevSha, ts, kind: draft.kind };
  if (draft.cart !== undefined) frame.cart = draft.cart;
  if (draft.delta !== undefined) frame.delta = draft.delta;
  frame.sig = sig;

  return { frame, seq, sha, canonical, fileName: `${seq}-${sha8(sha)}.json` };
}
